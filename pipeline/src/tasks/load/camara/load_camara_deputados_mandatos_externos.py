...
@task(
    task_run_name=TasksNames.CAMARA.LOAD.DEPUTADOS_MANDATOS_EXTERNOS,
    retries=APP_SETTINGS.CAMARA.TASK_RETRIES,
    retry_delay_seconds=APP_SETTINGS.CAMARA.TASK_RETRY_DELAY,
)
def load_camara_deputados_mandatos_externos(
    id_lote: int,
    ...
    if TasksNames.CAMARA.LOAD.DEPUTADOS_MANDATOS_EXTERNOS in ignore_tasks:
        logger.warning(
            f"A Task {TasksNames.CAMARA.LOAD.DEPUTADOS_MANDATOS_EXTERNOS} foi ignorada"
        )
        return
    if not mandatos_externos:
        logger.warning(
            f"Não foi possível executar a task '{TasksNames.CAMARA.LOAD.DEPUTADOS_MANDATOS_EXTERNOS}' pois o argumento do parâmetro 'mandatos_externos' é nulo"
        )
        return
