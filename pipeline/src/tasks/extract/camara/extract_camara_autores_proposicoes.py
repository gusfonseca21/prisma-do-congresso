...
def autores_proposicoes_urls(
    ...
    if not_downloaded_urls:
        logger.warning(
            f"A Tasks {TasksNames.CAMARA.EXTRACT.AUTORES_PROPOSICOES} possio URLs não baixadas nos lotes anteriores. Elas tentarão ser baixadas agora."
        )
...
async def extract_autores_proposicoes_camara(
    ...
    if TasksNames.CAMARA.EXTRACT.AUTORES_PROPOSICOES in ignore_tasks:
        logger.warning(
            f"A Task {TasksNames.CAMARA.EXTRACT.AUTORES_PROPOSICOES} foi ignorada"
        )
        return
    if use_files:
        logger.warning(
            f"O parâmetro 'use_files' é verdadeiro, a Task {TasksNames.CAMARA.EXTRACT.AUTORES_PROPOSICOES} irá retornar os dados à partir do arquivo em disco."
        )
        return load_ndjson(ExtractOutDir.CAMARA.AUTORES_PROPOSICOES)
    if not proposicoes_ids:
        logger.warning(
            f"Não foi possível executar a task '{TasksNames.CAMARA.EXTRACT.AUTORES_PROPOSICOES}' pois o argumento do parâmetro 'proposicoes_ids' é nulo"
        )
        return
