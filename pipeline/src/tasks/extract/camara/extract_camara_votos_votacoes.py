...
def votos_votacoes_urls(
    ...
    not_downloaded_urls = verify_not_downloaded_urls_in_task_db(
        TasksNames.CAMARA.EXTRACT.VOTOS_VOTACOES
    )

    if not_downloaded_urls:
        logger.warning(
            f"A Tasks {TasksNames.CAMARA.EXTRACT.VOTOS_VOTACOES} possio URLs não baixadas nos lotes anteriores. Elas tentarão ser baixadas agora."
        )
...
@task(
    task_run_name=TasksNames.CAMARA.EXTRACT.VOTOS_VOTACOES,
...
async def extract_votos_votacoes_camara(
    ...
    if TasksNames.CAMARA.EXTRACT.VOTOS_VOTACOES in ignore_tasks:
        logger.warning(
            f"A Task {TasksNames.CAMARA.EXTRACT.VOTOS_VOTACOES} foi ignorada"
        )
        return
    if use_files:
        logger.warning(
            f"O parâmetro 'use_files' é verdadeiro, a Task {TasksNames.CAMARA.EXTRACT.VOTOS_VOTACOES} irá retornar os dados à partir do arquivo em disco."
        )
        return load_ndjson(ExtractOutDir.CAMARA.VOTOS_VOTACOES)
    if not votacoes_ids:
        logger.warning(
            f"Não foi possível executar a task '{TasksNames.CAMARA.EXTRACT.VOTOS_VOTACOES}' pois o argumento do parâmetro 'votacoes_ids' é nulo"
        )
        return
    ...
    jsons = await fetch_many_jsons(
        ...
        task=TasksNames.CAMARA.EXTRACT.VOTOS_VOTACOES,
        lote_id=lote_id,
    )
