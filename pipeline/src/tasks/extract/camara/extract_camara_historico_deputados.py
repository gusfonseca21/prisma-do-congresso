...
def historico_deputados_urls(deputados_ids: list[int]) -> UrlsResult:
    ...
    if not_downloaded_urls:
        logger.warning(
            f"A Tasks {TasksNames.EXTRACT_CAMARA_HISTORICO_DEPUTADOS} possio URLs não baixadas nos lotes anteriores. Elas tentarão ser baixadas agora."
        )
...
async def extract_camara_historico_deputados(
    ...
    if use_files:
        logger.warning(
            f"O parâmetro 'use_files' é verdadeiro, a Task {TasksNames.EXTRACT_CAMARA_HISTORICO_DEPUTADOS} irá retornar os dados à partir do arquivo em disco."
        )
