...
def urls_despesas(
    ...
    not_downloaded_urls = verify_not_downloaded_urls_in_task_db(
        TasksNames.CAMARA.EXTRACT.DEPUTADOS_DESPESAS
    )

    if not_downloaded_urls:
        logger.warning(
            f"A Tasks {TasksNames.CAMARA.EXTRACT.DEPUTADOS_DESPESAS} possio URLs não baixadas nos lotes anteriores. Elas tentarão ser baixadas agora."
        )
...
@task(
    task_run_name=TasksNames.CAMARA.EXTRACT.DEPUTADOS_DESPESAS,
    retries=APP_SETTINGS.CAMARA.TASK_RETRIES,
    retry_delay_seconds=APP_SETTINGS.CAMARA.TASK_RETRY_DELAY,
)
async def extract_camara_deputados_despesas(
    ...
    if TasksNames.CAMARA.EXTRACT.DEPUTADOS_DESPESAS in ignore_tasks:
        logger.warning(
            f"A Task {TasksNames.CAMARA.EXTRACT.DEPUTADOS_DESPESAS} foi ignorada"
        )
        return
    if use_files:
        logger.warning(
            f"O parâmetro 'use_files' é verdadeiro, a Task {TasksNames.CAMARA.EXTRACT.DEPUTADOS_DESPESAS} irá retornar os dados à partir do arquivo em disco."
        )
        return load_ndjson(ExtractOutDir.CAMARA.DEPUTADOS_DESPESAS)
    if not deputados_ids:
        logger.warning(
            f"Não foi possível executar a task '{TasksNames.CAMARA.EXTRACT.DEPUTADOS_DESPESAS}' pois o argumento do parâmetro 'deputados_ids' é nulo"
        )
        return
    ...
    jsons = await fetch_many_jsons(
        ...
        task=TasksNames.CAMARA.EXTRACT.DEPUTADOS_DESPESAS,
        id_lote=id_lote,
    )

    save_ndjson(cast(list[dict], jsons), Path(ExtractOutDir.CAMARA.DEPUTADOS_DESPESAS))
