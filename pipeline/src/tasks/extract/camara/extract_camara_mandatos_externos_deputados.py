...
def mandatos_externos_deputados_urls(deputados_ids: list[int]) -> UrlsResult:
    ...
    if not_downloaded_urls:
        logger.warning(
            f"A Tasks {TasksNames.EXTRACT_CAMARA_MANDATOS_EXTERNOS_DEPUTADOS} possio URLs não baixadas nos lotes anteriores. Elas tentarão ser baixadas agora."
        )
...
async def extract_camara_mandatos_externos_deputados(
    ...
    if use_files:
        logger.warning(
            f"O parâmetro 'use_files' é verdadeiro, a Task {TasksNames.EXTRACT_CAMARA_MANDATOS_EXTERNOS_DEPUTADOS} irá retornar os dados à partir do arquivo em disco."
        )
