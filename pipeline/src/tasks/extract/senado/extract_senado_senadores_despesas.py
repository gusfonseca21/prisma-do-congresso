...
def despesas_senadores_urls(
    ...
    not_downloaded_urls = verify_not_downloaded_urls_in_task_db(
        TasksNames.SENADO.EXTRACT.SENADORES_DESPESAS
    )

    if not_downloaded_urls:
        logger.warning(
            f"A Tasks {TasksNames.SENADO.EXTRACT.SENADORES_DESPESAS} possio URLs não baixadas nos lotes anteriores. Elas tentarão ser baixadas agora."
        )
...
@task(
    task_run_name=TasksNames.SENADO.EXTRACT.SENADORES_DESPESAS,
    retries=APP_SETTINGS.SENADO.TASK_RETRIES,
    retry_delay_seconds=APP_SETTINGS.SENADO.TASK_RETRY_DELAY,
)
async def extract_senado_senadores_despesas(
    start_date: date,
    end_date: date,
    id_lote: int,
    use_files: bool,
    ...
    if TasksNames.SENADO.EXTRACT.SENADORES_DESPESAS in ignore_tasks:
        logger.warning(
            f"A Task {TasksNames.SENADO.EXTRACT.SENADORES_DESPESAS} foi ignorada"
        )
        return
    if use_files:
        logger.warning(
            f"O parâmetro 'use_files' é verdadeiro, a Task {TasksNames.SENADO.EXTRACT.SENADORES_DESPESAS} irá retornar os dados à partir do arquivo em disco."
        )
        jsons = load_ndjson(ExtractOutDir.SENADO.SENADORES_DESPESAS)
        return jsons
    ...
    jsons = await fetch_many_jsons(
        ...
        task=TasksNames.SENADO.EXTRACT.SENADORES_DESPESAS,
        id_lote=id_lote,
    )

    save_ndjson(cast(list[dict], jsons), ExtractOutDir.SENADO.SENADORES_DESPESAS)
