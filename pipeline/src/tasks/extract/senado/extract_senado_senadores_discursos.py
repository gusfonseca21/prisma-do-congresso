...
def discursos_senadores_urls(
    ...
    not_downloaded_urls = verify_not_downloaded_urls_in_task_db(
        TasksNames.SENADO.EXTRACT.SENADORES_DISCURSOS
    )

    if not_downloaded_urls:
        logger.warning(
            f"A Tasks {TasksNames.SENADO.EXTRACT.SENADORES_DISCURSOS} possio URLs não baixadas nos lotes anteriores. Elas tentarão ser baixadas agora."
        )
...
@task(
    task_run_name=TasksNames.SENADO.EXTRACT.SENADORES_DISCURSOS,
    retries=APP_SETTINGS.SENADO.TASK_RETRIES,
    retry_delay_seconds=APP_SETTINGS.SENADO.TASK_RETRY_DELAY,
)
async def extract_senado_senadores_discursos(
    ...
    if TasksNames.SENADO.EXTRACT.SENADORES_DISCURSOS in ignore_tasks:
        logger.warning(
            f"A Task {TasksNames.SENADO.EXTRACT.SENADORES_DISCURSOS} foi ignorada"
        )
        return
    if use_files:
        logger.warning(
            f"O parâmetro 'use_files' é verdadeiro, a Task {TasksNames.SENADO.EXTRACT.SENADORES_DISCURSOS} irá retornar os dados à partir do arquivo em disco."
        )
        jsons = load_ndjson(ExtractOutDir.SENADO.SENADORES_DISCURSOS)
        return jsons
    if not ids_senadores:
        logger.warning(
            f"Não foi possível executar a task '{TasksNames.SENADO.EXTRACT.SENADORES_DISCURSOS}' pois o argumento do parâmetro 'ids_senadores' é nulo"
        )
        return
    ...
    jsons = await fetch_many_jsons(
        ...
        task=TasksNames.SENADO.EXTRACT.SENADORES_DISCURSOS,
        id_lote=id_lote,
    )

    save_ndjson(cast(list[dict], jsons), ExtractOutDir.SENADO.SENADORES_DISCURSOS)
